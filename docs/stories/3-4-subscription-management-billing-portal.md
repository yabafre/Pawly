...
Story 3.4 implemented Subscription Management via Stripe Billing Portal. Extended StripeService with 3 new methods (createBillingPortalSession, getSubscriptionWithDetails, listInvoices). Extended stripe tRPC router with 2 protected procedures. Added invoice.paid webhook handler for payment recovery. Created billing page UI (RSC + BillingOverview client component) with Clinique Zen aesthetic. Added billing nav link. Code review applied 8 fixes (null assertion, dead code, missing tests, type safety). 324 total tests (58 new), build green.

### Files changed

**New files:**
- `packages/validators/src/stripe/billing.schema.ts`
- `packages/validators/src/stripe/billing.schema.test.ts`
- `apps/web/src/app/[locale]/admin/billing/page.tsx`
- `apps/web/src/app/[locale]/admin/billing/_actions/billing-actions.ts`
...
- `pnpm test`: 120 API + 79 validators + 125 web = 324 total, 0 failures
- `pnpm build`: green
- 58 new tests across 4 test suites

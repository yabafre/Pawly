...
- [x] **Then** the directory structure is created according to the architecture decisions
  ...
  - **And** core models (`User`, `MagicLink`) include a mandatory `clinicId` field
  - **And** `@pawly/validators`, `@pawly/types`, and `@pawly/zod` packages are initialized in `packages/`.
...
- [x] Create `apps/api` using Nest CLI: `nest new apps/api --package-manager pnpm`
- [x] Create `apps/web` using Next CLI: `npx create-next-app@latest apps/web --typescript --tailwind --eslint --app --src-dir`
- [x] Initialize Turbo: `npx turbo init`
- [x] Install Prisma: `pnpm add -D prisma` and `pnpm add @prisma/client`
- [x] Create `apps/api/prisma/schema/` directory
- [x] Move `generator` and `datasource` to `base.prisma`
- [x] Create `User.prisma` and `MagicLink.prisma` with `clinicId`
- [x] Update `package.json` in `apps/api` to point to the schema folder
- [x] Create `packages/validators`, `packages/types`, and `packages/zod`
- [x] Ensure they are correctly exported and linked in the monorepo
- [x] Run `pnpm prisma validate` in `apps/api` to ensure schema folder merging works
- [x] Run `turbo build` to verify monorepo orchestration

...
> - **Behaviour change accepted.** After this story a **HARD** `CONTRACT_COMPLIANCE` / `ROTATION_EQUITY` rule really blocks publication (the fix). A clinic with such a rule and a non-compliant published-candidate month will see publish refused. Existing specs asserting "contract/rotation = soft-only" are updated in Tasks 4/5/6. **Two additions at review (aped-review 2026-07-12):** (a) the validation path now respects `applicableJobTypes` on ROTATION_EQUITY like generation and moves already did — the legacy validator counted every targetDay shift regardless of jobType (4th accepted change, finding M1, pinned by test); (b) monthly contract thresholds compare minute-precise net totals — the legacy round-then-compare hid up to 29 minutes of overrun (finding m6, pinned by boundary test).
> - **Validation contract semantics.** ~~Weekly is evaluated only when `maxWeeklyHours` is set … the validator does not invent a `contractHours` cap when no threshold is configured.~~ **Superseded at review (finding m3, decision Alex; refined at fix-verification):** for a CONTRACT_COMPLIANCE rule that declares **at least one hour cap**, the weekly check always runs with effective limit `min(contractHours, maxWeeklyHours)` falling back to `contractHours` — closing the original m3 hole (a maxMonthly-only HARD rule let a week the generator/move refused validate green). A rule declaring **no** hour cap (the seeded 11-3 statutory row, rest-only rules) still evaluates nothing here: `contractHours` is a contractual base, not a legal limit — without this guard every onboarded clinic's publication was gated at 35h/week under the "French labor-law limits" label (Edge fix-verification, MAJOR). **Known residual (follow-up ticket):** generation and `preValidateMove` apply the `contractHours` weekly floor for *every* CONTRACT_COMPLIANCE rule including the capless statutory row (pre-existing since 11-3) — so gen/move remain stricter than validation for capless rules; re-scoping that floor is out of this story.
...
- **Headed re-verification of the review fixes (2026-07-12T19:20Z, next-browser headless Chromium, web:3020/API:3001, clinic "Clinique Zen Dev" = seed admin's own — no repoint):** created a HARD `CONTRACT_COMPLIANCE {maxWeeklyHours:20}` rule; employee Camille was at 35h net in two ISO weeks. Health Bar → **"2 conflits, Aucun avertissement, 80% prêt"** and **"Publication impossible — résolvez les conflits d'abord"**. Popover text: **"Camille Martin — 35h la semaine du 13/07/2026, dépasse la limite hebdomadaire de 20h"** — the **m2 fix live**: `messageParams.date` renders `13/07/2026` (FR), no ISO leak, no raw key visible. React component props (`PlanningHealthBar`) confirmed `hardViolationCount:2, softViolationCount:0`; both violations `severity:"blocking"`, `messageKey:"violations.contractCompliance.weeklyOvertime"`, `messageParams.date:"13/07/2026"` while `affectedDate:"2026-07-13"` stays ISO (grid key) — **m2 + HARD-surfacing + m4 (soft:[]) all confirmed end-to-end**. Move path: a `preValidateMove` drag-over returned `hard:[{rule:"CONTRACT_COMPLIANCE", message:"Overtime risk: 35h this week, effective limit 20h"}]` — **same shared-engine verdict + same effective limit 20h as the post-hoc path (cross-path consistency, AC1)**. Deleting the rule returned the Health Bar to green ("Tout est bon — aucune violation détectée"). No data mutated (10 shifts intact, all MANUAL; `preValidateMove` is read-only, no drop committed); test rule deleted; clinic restored to 0 rules.

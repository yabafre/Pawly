...
> - **Validation contract semantics.** ~~Weekly is evaluated only when `maxWeeklyHours` is set … the validator does not invent a `contractHours` cap when no threshold is configured.~~ **Superseded at review (finding m3, decision Alex):** the weekly check always runs for a CONTRACT_COMPLIANCE rule, with effective limit `min(contractHours, maxWeeklyHours)` falling back to `contractHours` — this is what actually mirrors `preValidateMove` and the generator (both always applied the `contractHours` floor; the original wording claimed symmetry the code did not have). Consequence: the 11-3 statutory HARD rule now also enforces the weekly `contractHours` floor on the validation path, exactly as it already did in generation and moves — a roster over contract hours can no longer validate green and publish.

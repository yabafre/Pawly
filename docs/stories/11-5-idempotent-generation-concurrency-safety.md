...
- **Live UI journey (next-browser, headed, real worktree servers with the fix — API:3001, web:3020):** logged in as admin, ran a real month generation on "Clinique test" (5 employees, "semaine standard" template, July 2026) → toast **"Planning généré avec succès"** (95% ready, 0 conflict, 0 hole), **zero runtime errors**. Network showed the generation mutation as a **single POST** (`next-action` fired once, 200, ~1.2s — `fetchWithRetry` did not replay it: AC1 on the real web→tRPC→API path). DB after: **85 GENERATED shifts committed** by the advisory-locked transaction, **0 duplicate `(employee_id, date, start_time)`** (AC3 net holds). Confirms the READ COMMITTED fix is non-regressive end-to-end on the real app. (Turnstile's widget does not init inside the Playwright context, so login was done via a minted admin JWT cookie; the `clinicId` is cached in Redis `user:{id}` for 5 min by `jwt.strategy.validate`, flushed to repoint. Test data restored afterward: admin clinic reset, 85 shifts deleted, cache flushed.)

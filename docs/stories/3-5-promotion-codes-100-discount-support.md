...
Story 3.5 implemented Promotion Codes & 100% Discount Support. Created `deriveEntitlementTier()` utility to fix hardcoded entitlementTier across all checkout/subscription flows. Enhanced checkout.session.completed webhook handler to capture promotion data (promotionCodeId, couponId, discountType, discountValue, couponMetadataType) with graceful degradation. Extended getBillingOverview to return promotion fields. Updated BillingOverview UI with Clinique Zen promotion display section. 30 new tests (8 utility + 6 webhook + 2 router + 14 validators), 354 total passing, build green.

### Files changed

**New files (4):**
- `apps/api/src/modules/stripe/stripe.utils.ts`
- `apps/api/src/modules/stripe/stripe.utils.spec.ts`
- `packages/validators/src/stripe/promotion.schema.ts`
- `packages/validators/src/stripe/promotion.schema.test.ts`
...
- `apps/api/src/trpc/routers/stripe.router.spec.ts`
- `packages/validators/src/stripe/billing.schema.ts`
...
None. All tasks completed as specified. Graceful degradation added to coupon retrieval in webhook (non-critical failure should not abort Clinic/Admin/Subscription creation).

### Test output

- `pnpm test`: 136 API + 93 validators + 125 web = 354 total, 0 failures
- `pnpm build`: green
- 30 new tests: 8 utility + 6 webhook + 2 router + 14 validators

...
1. **[AC1 — O(1) rotation index, UNCONDITIONAL]** **Given** a stress configuration (50 employees, 24/7 clinic, 3 shift types, 31-day month), **When** a month is generated, **Then** the three rotation-equity evaluators — HARD `violatesHardRotationEquity`, SOFT-scoring `countTargetDayShifts`, SOFT-violation `checkRotationEquity` — resolve each employee's target-day count via a **per-`(employee, ISO-weekday)` index maintained incrementally** (seeded from border + surviving shifts, incremented on every assignment — the FIX-4 counter pattern) plus a **`quarterlyDayOfWeekCounts` index pre-built once** from `constraints.quarterlyShifts`, **instead of** re-filtering the flat `alreadyAssigned` array (allocating a `Date` per element) per employee per slot per rule. **And** the generation output — assignments, holes, `violations.hard`, `violations.soft`, `stats` — is **identical** to the pre-index scan implementation (bit-for-bit equivalence; the index reflects the same multiset as `alreadyAssigned`), so determinism (`score → #shifts → #weekends → employeeId`) is preserved.

2. **[AC2 — event loop never blocked, SPIKE-GATED]** **Given** a full-month generation, **When** the slot-assignment loop runs, **Then** it never holds the Node event loop for the whole generation:
   - **If Task-1 spike = OK:** `generateMonthlyPlan` runs inside a Trigger.dev job (`generate-monthly-plan` task, Nest standalone context). `planning.generatePlan` enqueues the job and returns `{ runId, publicAccessToken }`; the admin UI subscribes via `useRealtimeRun` and renders the `GenerationResult` from `run.output` on completion. The Story-11-1 published-change guard stays enforced **synchronously in the router** (immediate `PUBLISHED_CHANGE_REQUIRES_ACK` before enqueue) so the acknowledgement dialog is preserved.
   - **If Task-1 spike = KO:** the slot loop yields to the event loop every N slots via `await new Promise((resolve) => setImmediate(resolve))`, in-process. Output unchanged (the yield does not reorder deterministic processing).

3. **[AC3 — NFR2 / NFR9]** **Given** the stress configuration, **When** the month is generated, **Then** a load test measures the generation-core duration, logs it, and asserts it completes under the NFR2 budget (`< 2000ms`, wide anti-flaky threshold) at 50 employees without degradation (NFR9). The AC1 equivalence tests are the functional proof that the optimisation changed nothing but speed.

**FRs covered:** (perf hardening — no new FR). **NFRs:** NFR2 (<2s + loading feedback >1s), NFR9 (50 employees), NFR10 (concurrent generations — async branch).
...
- **Equivalence is the AC1 proof.** The **pre-existing** rotation tests staying green after the rewrite proves the index ≡ scan; the 3 new T5b tests pin the HARD-block / SOFT-penalty / SOFT-violation paths explicitly (RED against a broken index, GREEN with the correct one).

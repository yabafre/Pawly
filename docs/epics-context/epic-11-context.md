...
### Story 11-6 — done 2026-07-10T14:29:02Z

- **Decisions:** The three single-shift amendment paths (move/create/delete) now commit their shift mutation + `recordAmendment` in one interactive `this.prisma.$transaction`, with `notifyScheduleChange` moved AFTER commit (fire-and-forget) — generalizing the `confirmPresence` pattern (invariant #5). The PUBLISHED re-check deliberately stays OUTSIDE the tx (advisory-lock TOCTOU is 11-5/KON-122's scope, locked with Alex). Router schedule-cache invalidation (`schedule:*` / `planning:pub:*` / `dashboard:stats`) now runs in a `try/finally` on all 5 shift-mutation procedures via a swallow-and-log `invalidateScheduleCaches` helper, so a handler throw or a Redis outage never leaves a stale cache and never masks the result.
- **Files:** `apps/api/src/modules/planning/planning-generation.service.ts`(+spec); `apps/api/src/trpc/routers/planning.router.ts`(+spec).
- **Contracts:** `recordAmendment` signature is now `(tx: Prisma.TransactionClient, clinicId, months)` — the 2 bulk callers (`generateMonthlyPlan`, `deleteGeneratedShifts`) pass `this.prisma`; the amendment paths pass the interactive `tx`. Any new shift-mutating path on a PUBLISHED month must (a) wrap mutation + `recordAmendment(tx, …)` in one `$transaction` and notify post-commit, and (b) be invalidated by the router's `try/finally` (reuse `invalidateScheduleCaches`). Bulk-amendment atomicity is intentionally NOT transactional (11-1 semantics unchanged).
- **Deviations from plan:** none in implementation (all find/replace anchors matched verbatim). aped-review added 4 tests in-review — AC2 notify-outage-doesn't-block for the 3 single paths + AC3 `deleteGeneratedShifts` throw-invalidation (now 5/5 procedures) — and tightened one assertion (`toHaveBeenCalledTimes(1)`). Non-blocking follow-ups: L2 real-DB atomicity check (the default `$transaction` mock's `tx === this.prisma` can't regression-test rollback); optional perf (skip invalidation on the `PUBLISHED_CHANGE_REQUIRES_ACK` throw path, or `KEYS`→`SCAN`). 24 pre-existing full-`tsc` errors in unrelated spec files flagged for a separate cleanup story.

...
### Story 11-5 — done 2026-07-10T00:00:00Z

- **Decisions:** Idempotency/concurrency for the two bulk paths. **AC1**: the server-side tRPC transport `fetchWithRetry` (`apps/web/src/lib/trpc/client.ts`) retries ONLY `GET` (queries); any non-GET (mutation) gets a single attempt — a reverse-proxy 502/504 or double-click can no longer replay `generateMonthlyPlan`/`publishPlan` from Next.js. **AC2/AC3**: both `generateMonthlyPlan` and `publishPlan` take the SAME `pg_advisory_xact_lock(hashtext(clinicId), hashtext(month))` as the first statement of their interactive `$transaction`. **aped-review changed the isolation from Serializable to the default READ COMMITTED** — under SERIALIZABLE the tx snapshot freezes at the advisory-lock SELECT (before the lock is granted), so the 2nd same-key waiter would NOT see the 1st run's committed rows (spurious P2034/P2002); READ COMMITTED gives the clean "wait → see committed rows → delete+recreate" happy path. `timeout: 15000` + a bounded `withSerializationRetry` (retries only P2034, ≤3×) are kept as a residual deadlock net. **11-6 and any future locked path: use READ COMMITTED + advisory lock, NOT Serializable.**
- **Files:** `apps/web/src/lib/trpc/client.ts` (+ new `client.spec.ts`); `apps/api/src/modules/planning/planning-generation.service.ts` (+ `planning-generation.service.spec.ts`). No schema/migration (the `@@unique` shipped in 11-2).
- **Contracts:** `fetchWithRetry` is now exported from `client.ts`. The two locked transactions run at READ COMMITTED with `{ timeout: 15000 }` and no `isolationLevel`. Any new hand-rolled `tx` mock flowing through generate/publish MUST stub `$executeRaw` (the advisory lock is the first tx statement) or the real code throws `tx.$executeRaw is not a function`.
- **Deviations from plan:** aped-review flipped Serializable→READ COMMITTED (Edge auditor MAJOR; the story's own "second's deleteMany sees the first's committed rows" happy path is only true under READ COMMITTED). Added tx-options assertions (`{ timeout: 15000 }`) + a 3×-P2034 exhaustion test. Live L2 verified against real Neon: advisory lock serialized the 2nd run (~1.18 s wait); READ COMMITTED → one month (3 rows), no P2002; SERIALIZABLE (old code) → 2nd run rejected 40001, confirming the finding. API 152/152 on the planning-generation spec.

...
- Employee dashboard portal created at `/[locale]/dashboard/` with EMPLOYEE role guard and subscription check
- School day declarations use existing `Unavailability` model with `type: SCHOOL` — zero schema changes
- Self-service enforcement ensures apprentices can only manage their own declarations
- Admin notification via fire-and-forget Resend emails after successful declaration
- Monthly cron reminder at 9 AM on the 25th (Europe/Paris) for undeclared apprentices
- Calendar uses react-day-picker v9 with `mode="multiple"` restricted to next month
- EmployeeContext React context propagates employeeId/jobType from server layout to client components
- Admin-side visibility via school day badge on EmployeeCard for APPRENTICE employees
- Employee invitation flow: admin creates employee with email → User+Employee created atomically → activation email sent → employee can log in

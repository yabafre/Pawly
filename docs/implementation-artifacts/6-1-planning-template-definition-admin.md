...
- All quality gates passed: `pnpm test` (1130 total: 328 validators, 408 API, 394 web) and `pnpm build` green.
- New tests: 35 validators + 19 API service + 17 API router + 21 web = 92 new tests.
- Template data structure: `TemplateData { days: TemplateDay[] }` → `TemplateDay { dayOfWeek: 1-7, slots: TemplateSlot[] }` → `TemplateSlot { shiftTypeCode, requiredStaff, requiredJobTypes? }`.
- PlanningTemplateService validates shiftTypeCode cross-references against ClinicShiftType and JobType enum at service layer.
- Zod `.refine()` for unique dayOfWeek kept separate from base schema per project pattern.
- **Code review fixes (2026-02-13):** Fixed DRY violation (JOB_TYPE_VALUES → import from validators), added data validation to duplicateTemplate, removed redundant dayOfWeek check from service (Zod handles it), fixed 3 hardcoded English strings in TemplateEditor, added aria-label for dropdown trigger + form validation announcements, added 10 TemplateEditor tests, fixed FR deleteFailed typo.

### Change Log

- **validators**: Created planning-template schemas (slot, day, data, CRUD), 35 tests
- **api/service**: Created PlanningTemplateService (CRUD + duplicate + validate), 19 tests
- **api/router**: Added 6 template procedures to planning router, 17 tests
- **api/module**: Registered PlanningTemplateService in PlanningModule, TRPCServices, TRPCMiddleware
- **web/actions**: Created 6 server actions for template CRUD
- **web/hooks**: Created useTemplates hook with query + 4 mutations
- **web/ui**: Created TemplateList, TemplateEditor, TemplateSlotForm, TemplateWeekPreview, page/loading/error
- **web/nav**: Added Templates link with LayoutTemplate icon
- **web/i18n**: Added admin.planningTemplates namespace in en.json and fr.json (~70 keys each)
- **web/ui**: Installed shadcn dropdown-menu component
- **review-fix**: DRY — imported JOB_TYPES from @pawly/validators in service (was hardcoded)
- **review-fix**: duplicateTemplate now validates data integrity before creating copy
- **review-fix**: Removed redundant dayOfWeek uniqueness check (Zod handles via .refine())
- **review-fix**: Replaced 3 hardcoded English strings with i18n keys in TemplateEditor
- **review-fix**: Fixed hardcoded "staff" in TemplateWeekPreview tooltip
- **review-fix**: Added aria-label on dropdown menu trigger (WCAG AA)
- **review-fix**: Added aria-invalid + sr-only error announcement for name input
- **review-fix**: Added 10 TemplateEditor component tests (create/edit title, 7 day rows, name pre-fill, save disabled, submit, cancel, workDays dimming, add slot)
- **review-fix**: Fixed FR translation typo: deleteFailed said "dupliquer" instead of "supprimer"
- **review-fix**: Added i18n keys: actions.menu, form.nameRequired (EN + FR)

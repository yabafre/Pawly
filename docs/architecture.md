...
_This document builds collaboratively through step-by-step discovery. Sections are appended as we work through each architectural decision together (originally captured as the Architecture Decision Document)._
...
_No adversarial design councils were dispatched; this architecture predates the APED council process._
...
- **@pawly/validators**: Shared Zod schemas used by both tRPC input validation and Zsa Server Action validation. Single source of truth for data shape.
- **@pawly/types**: Shared TypeScript types and interfaces.
- **@pawly/zod**: Shared Zod single instance across the monorepo (avoids version mismatches).
- **@pawly/config**: Shared ESLint/TS configs.
...
- **W1 — Stripe webhook idempotency**: Subscription state is webhook-driven; duplicate `event.id`s must be rejected via the `StripeEvent` table before processing, and HMAC verification (`stripe.webhooks.constructEvent()`) must guard every event. Raw body parser must remain scoped to `/api/stripe/webhook` only — never app-wide.
- **W2 — Subscription gating order**: Admin routes require auth AND active subscription AND completed onboarding; Stripe webhooks must be operational before admin routes are protected, and the DB-synced Stripe status (never the frontend) is the source of truth.
- **W3 — Offline cache staleness**: Employees have offline read-only PWA access; cached planning data can go stale and must be reconciled against server state on reconnect.
- **W4 — i18n proxy ordering**: The `next-intl` proxy must run locale detection/redirect before any auth or subscription guard; misordering breaks page routing. Proxy must be configured before any page routing works correctly.
- **W5 — clinicId resolution**: `clinicId` must always be resolved from the DB (`findUnique({ email })`) and carried in the JWT payload — never accepted from client input, and `NEXT_PUBLIC_CLINIC_ID` must stay eliminated.
- **W6 — Registration abuse**: The self-service `register` endpoint creates Clinic + Admin + Subscription atomically; Turnstile CAPTCHA and rate limiting must protect it against abuse.
- **W7 — Promo / entitlement safety**: The `entitlementTier` field must gate paid features so future paid features can be added without breaking 100% promo users; promos are tracked via Stripe metadata.


## Phase 7 — Residual Gaps

_No open architectural questions remain; the epics below are fully specified._


## Phase 8 — Epic Zero

- **E0.1** — Prisma Migration (Schema Folders) + Core models. ✅ DONE (Epic 1, Story 1.1).
- **E0.2** — NestJS + tRPC + Zsa Foundation. ✅ DONE (Epic 1, Story 1.1).
- **E0.3** — Magic Link Authentication (JWT + Magic Link backend + Login UI). ✅ DONE (Epic 1, Stories 1.2–1.3).

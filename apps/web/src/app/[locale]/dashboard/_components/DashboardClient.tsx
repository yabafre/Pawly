  ...
  return (
    ...
    >
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <span
          className={`text-sm font-medium transition-opacity ${validated ? 'text-white' : 'text-muted-foreground'}`}
        >
...
const EmployeeDashboard = () => {
  ...
  const [pageState, dispatch] = useReducer(
    (
      state: { isMounted: boolean; showSplash: boolean },
      action: Partial<{ isMounted: boolean; showSplash: boolean }>
    ) => ({ ...state, ...action }),
  ...
  const weekSummary = scheduleData?.weeklySummary.find((w) => w.weekNumber === currentWeekNumber);
  const weekHours = weekSummary ? Math.round((weekSummary.totalMinutes / 60) * 10) / 10 : 0;
  const contractHours = scheduleData?.employee.contractHours ?? 35;
  const progressPercent =
    contractHours > 0 ? Math.min(Math.round((weekHours / contractHours) * 100), 100) : 0;
    ...
    <div className="space-y-6 pb-8">
        ...
        <div className="bg-card rounded-3xl p-5 border flex flex-col justify-between h-40">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              <div
                className={`w-1.5 h-1.5 rounded-full ${progressPercent >= 100 ? 'bg-emerald-500' : 'bg-primary'}`}
              />
      ...
      {todayShift && (
        <div>
          ...
          <div className="bg-card rounded-3xl p-5 border" data-tour="employee-today">
            <div
              className={`flex justify-between items-start ${!todayShift.isConfirmed ? 'mb-5' : ''}`}
            >
              <div className="flex gap-3">
                <div
                  className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0"
                  style={{
                    backgroundColor: todayShiftType?.color
                      ? `${todayShiftType.color}15`
                      : 'var(--muted)',
          ...
          <div className="space-y-2">
            {nextDays.map((entry) => {
              ...
              if (entry.type === 'unavailability' && entry.unavailability) {
                ...
                const colorClass =
                  UNAVAILABILITY_COLORS[entry.unavailability.type] ??

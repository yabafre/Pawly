...
describe('PlanningRuleFormSheet', () => {
  it('renders create form when open with no editing rule', () => {
    render(<PlanningRuleFormSheet open={true} onClose={vi.fn()} editingRule={null} />);
  ...
  it('renders edit form when editing rule provided', () => {
    ...
    render(<PlanningRuleFormSheet open={true} onClose={vi.fn()} editingRule={editingRule} />);
  ...
  it('does not render when closed', () => {
    const { container } = render(
      <PlanningRuleFormSheet open={false} onClose={vi.fn()} editingRule={null} />
    );
...
describe('PlanningRuleConfigEditor', () => {
  ...
  it('renders CONTRACT_COMPLIANCE config fields', () => {
    ...
    // KON-144 — the engine has honoured maxDailyHours since KON-140; without this field
    // the capability was unreachable from the UI.
    expect(screen.getByText('config.maxDailyHours')).toBeDefined();
    expect(screen.getByText('config.maxDailyHoursHint')).toBeDefined();
  });

  it('KON-144 — edits maxDailyHours through the same optional-number path as the other caps', async () => {
    ...
    const daily = screen.getByText('config.maxDailyHours').closest('div')!.querySelector('input')!;
    // Only a TIGHTENING is useful: >= 12h is redundant with the statutory ceiling and the
    // engine ignores it, so the input must not invite values the engine will drop.
    expect(daily.getAttribute('max')).toBe('11.9');

    fireEvent.change(daily, { target: { value: '8' } });
  ...
  it('KON-144 — clearing the daily cap removes it instead of sending 0', () => {
    const onChange = vi.fn();
    // Seeded WITH a value: the input is controlled, so clearing an already-empty field
    // fires nothing — the cleared state has to start from a set one.
    render(
      <PlanningRuleConfigEditor
        category="CONTRACT_COMPLIANCE"
        config={{ maxWeeklyHours: 35, maxDailyHours: 8 }}
        onChange={onChange}
      />
    );

    const daily = screen.getByText('config.maxDailyHours').closest('div')!.querySelector('input')!;
    expect(daily.getAttribute('value') ?? daily.value).toBe('8');
  ...
  it('returns null for unknown category', () => {
    const { container } = render(
      <PlanningRuleConfigEditor category={'UNKNOWN' as any} config={{}} onChange={vi.fn()} />
    );

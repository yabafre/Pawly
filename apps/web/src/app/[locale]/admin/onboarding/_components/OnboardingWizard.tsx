...
import {
  setupStarterSubscriptionAction,
  syncAfterCheckoutAction,
} from '@/app/[locale]/admin/billing/_actions/billing-actions';
...
function OnboardingWizardForm({
  ...
  initialData: OnboardingInitialData & { onboardingDraft?: Record<string, unknown> | null };
}) {
  const t = useTranslations('onboarding');
  ...
  const draft = initialData.onboardingDraft as {
    step?: number;
  ...
  const fallbackDefaults = {
    ...
    workDays: (initialData.config?.workDays as WorkDay[]) ?? [
      'MONDAY',
      'TUESDAY',
      'WEDNESDAY',
      'THURSDAY',
  ...
  const form = useForm({
    defaultValues: draft?.values ? { ...fallbackDefaults, ...draft.values } : fallbackDefaults,
  ...
  const handleNext = () => {
    if (validateCurrentStep()) {
      ...
      toast.error(t('errors.incompleteStep'));
    }
  };

...
const StatCard = ({
  ...
  <div
    data-tour={dataTour}
    className="bg-card rounded-2xl border border-border p-5 flex flex-col justify-between gap-4"
  >
...
export function DashboardPageClient() {
  ...
  const teamHelper = stats ? t('teamBreakdown', { count: stats.activeEmployees }) : '';
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-foreground">{t('overview')}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        <StatCard
          dataTour="admin-pending-requests"
          ...
          linkLabel={stats && stats.pendingVariances > 0 ? t('goToVariance') : t('goToAbsences')}
        />
        <StatCard
          title={t('activeTeam')}
      ...
      {/* Today's team */}
      <div data-tour="admin-todays-team" className="bg-card rounded-2xl border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold text-foreground">{t('todayTeam')}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {stats.todayEmployees.map(
              (emp: {
                ...
                <div key={emp.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50">
